  ...
  return (
    <div className="flex flex-col md:flex-row h-screen w-full bg-[#020617] text-slate-200 overflow-hidden font-sans">
      ...
      <main className="flex-1 relative flex flex-col min-w-0 overflow-hidden">
        ...
        <header className="md:hidden flex items-center justify-between p-5 bg-[#020617] border-b border-slate-800/50 z-20">
      ...
      <div className="md:hidden flex bg-[#0f172a] border-t border-slate-800/50 p-2 justify-around shadow-[0_-10px_30px_rgba(0,0,0,0.5)] z-20">
        <button 
          onClick={() => setActiveView(AppView.SORA_STUDIO)}
          className={`flex flex-col items-center p-3 gap-1 transition-all rounded-xl ${activeView === AppView.SORA_STUDIO ? 'text-cyan-400 bg-cyan-500/5' : 'text-slate-500'}`}
        >
        ...
        <button 
          ...
          className={`flex flex-col items-center p-3 gap-1 transition-all rounded-xl ${activeView === AppView.HISTORY ? 'text-cyan-400 bg-cyan-500/5' : 'text-slate-500'}`}
        >
