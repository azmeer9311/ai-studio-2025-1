    ...
    <div className="flex flex-col h-full bg-[#020617] p-4 md:p-12 overflow-y-auto custom-scrollbar">
      <div className="max-w-6xl mx-auto w-full">
        <header className="mb-8 md:mb-12 flex flex-col items-center text-center">
          <div className="mb-4 md:mb-6 flex items-center gap-3 px-4 py-1.5 rounded-full bg-cyan-500/10 text-cyan-400 text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] border border-cyan-500/20 shadow-[0_0_20px_rgba(34,211,238,0.05)]">
            <img src={logoUrl} alt="Logo" className="w-4 h-4 md:w-5 md:h-5 object-contain logo-glow-animate" />
            <span>Azmeer AI • Sora 2.0</span>
          </div>
          <h2 className="text-3xl md:text-5xl font-black text-white tracking-tighter mb-3 md:mb-4">
            UGC <span className="text-cyan-500">STUDIO</span>
          </h2>
          <p className="text-slate-500 max-w-lg text-[11px] md:text-sm font-medium leading-relaxed px-4">
            Pilih config dan taip prompt produk hampa untuk jana video kualiti tinggi.
          </p>
        </header>
        ...
        {isLocked && (
          <div className="mb-6 p-4 md:p-6 bg-rose-500/10 border border-rose-500/20 rounded-2xl md:rounded-3xl text-center">
            <p className="text-[9px] md:text-[10px] font-black text-rose-500 uppercase tracking-widest">
              Sila tunggu pengesahan Admin sebelum boleh guna Studio.
            </p>
          </div>
        )}
        ...
        <div className={`grid grid-cols-1 xl:grid-cols-12 gap-6 md:gap-8 items-start ${isLocked ? 'opacity-50 pointer-events-none grayscale' : ''}`}>
          <div className="xl:col-span-5 space-y-5 md:space-y-6">
            ...
            {/* UGC Config Interface */}
            <div className="bg-slate-900/40 border border-slate-800 rounded-[2rem] p-5 md:p-6 space-y-4 md:space-y-5">
              <h3 className="text-[10px] font-black text-cyan-500 uppercase tracking-widest px-1">AI Video Config</h3>
            ...
            <div className="bg-[#0f172a]/60 backdrop-blur-xl border border-slate-800/50 p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] shadow-2xl relative">
              <div className="space-y-6">
                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4 px-1">Rujukan Gambar (Optional)</label>
                  {!filePreview ? (
                    <div onClick={() => !isLocked && fileInputRef.current?.click()} className="w-full border-2 border-dashed border-slate-800 rounded-2xl p-5 md:p-6 flex flex-col items-center justify-center gap-2 cursor-pointer hover:border-cyan-500/30 hover:bg-cyan-500/5 transition-all">
                      ...
                      <span className="text-[9px] font-bold text-slate-600 uppercase">Upload Media</span>
                    </div>
                  ) : (
                    <div className="relative rounded-2xl overflow-hidden aspect-video border border-slate-800">
                      <img src={filePreview} alt="Preview" className="w-full h-full object-cover" />
                      <button onClick={clearFile} className="absolute top-2 right-2 bg-red-500 p-1.5 rounded-full text-white shadow-lg"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M6 18L18 6M6 6l12 12" /></svg></button>
                    </div>
                  )}
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4 px-1">Video Prompt</label>
                  <textarea
                    value={prompt}
                    onChange={(e) => setPrompt(e.target.value)}
                    disabled={isLocked}
                    placeholder="Taip deskripsi produk hampa kat sini..."
                    className="w-full h-36 md:h-44 bg-slate-950/80 border border-slate-800 rounded-2xl p-4 md:p-5 text-[11px] md:text-xs text-slate-200 outline-none focus:border-cyan-500/50 transition-all resize-none custom-scrollbar font-medium leading-relaxed"
                  />
                </div>
              </div>

              {/* Butang Prompt Generator di atas butang Generate Video */}
              <div className="mt-6 flex flex-col gap-3">
                <button 
                  onClick={handleMagicGenerate}
                  disabled={isLocked || isWizardLoading || isGenerating}
                  className="w-full py-3.5 bg-slate-900 border border-slate-800 hover:border-cyan-500/30 text-slate-400 hover:text-cyan-400 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 active:scale-95 shadow-lg"
                >
                  {isWizardLoading ? <div className="w-3.5 h-3.5 border-2 border-cyan-500/20 border-t-cyan-500 rounded-full animate-spin"></div> : <svg className="w-3.5 h-3.5" fill="currentColor" viewBox="0 0 20 20"><path d="M11.3 1.046A1 1 0 0112 2v5h4a1 1 0 01.82 1.573l-7 10A1 1 0 018 18v-5H4a1 1 0 01-.82-1.573l7-10a1 1 0 011.12-.38z" /></svg>}
                  {isWizardLoading ? 'Jana Skrip...' : 'Generate UGC Script'}
                </button>
          ...
          <div className="xl:col-span-7 flex flex-col items-center">
            <div className={`w-full aspect-video rounded-[2rem] md:rounded-[2.5rem] overflow-hidden bg-[#0f172a]/40 backdrop-blur-xl border border-slate-800/50 flex items-center justify-center relative shadow-2xl transition-all duration-700 ${aspectRatio === 'portrait' ? 'max-w-[320px] md:max-w-[340px] aspect-[9/16]' : 'w-full'}`}>
              {renderedVideoUrl ? (
                ...
                <div className="text-center space-y-4 md:space-y-6 flex flex-col items-center p-8 md:p-12">
                  <div className="relative">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center text-white font-black text-base md:text-lg">{progress}%</div>
                  </div>
                  <p className="text-cyan-500 font-black uppercase tracking-[0.3em] text-[9px] md:text-[10px] animate-pulse">Rendering Engine</p>
                </div>
              ) : (
                <div className="text-center p-8 md:p-12 opacity-20">
                   <div className="w-12 h-12 md:w-16 md:h-16 mx-auto mb-4 border-2 border-dashed border-slate-700 rounded-full flex items-center justify-center">
                      <svg className="w-5 h-5 md:w-6 md:h-6 text-slate-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M14.752 11.168l-3.197-2.132A1 1 0 0010 9.87v4.263a1 1 0 001.555.832l3.197-2.132a1 1 0 000-1.664z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                   </div>
                   <p className="text-[9px] md:text-[10px] font-black uppercase tracking-widest text-slate-400">Pratonton Video</p>
                </div>
              )}
            </div>
